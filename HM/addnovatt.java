...
public class addnovatt {
	...
	public static void mainNewScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					addnovatt window = new addnovatt();
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBounds(100, 100, 806, 473);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.getContentPane().setLayout(null);
		JLabel lblAddJanuaryAttendence = new JLabel("add November attendence");
		lblAddJanuaryAttendence.setFont(new Font("Tahoma", Font.BOLD, 25));
		lblAddJanuaryAttendence.setBounds(133, 88, 371, 37);
				...
				if (e.getSource() == btnOk) {
					...
					try {
						...
						java.sql.PreparedStatement ps = con.prepareStatement(
								"insert into addnovematt(studentid,studentname,day,year,status) values(?,?,?,?,?)");
						ps.setString(1, dn);
						ps.setString(2, dn1);
						ps.setString(3, dn2);
						ps.setString(4, dn3);
		...
		btnCancel.setBounds(291, 388, 89, 21);
