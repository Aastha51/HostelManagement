...
public class addvissept {
	...
	public static void mainNewScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					addvissept window = new addvissept();
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBounds(100, 100, 806, 474);
				...
				if (e.getSource() == btnOk) {
					...
					try {
						...
						java.sql.PreparedStatement ps = con.prepareStatement(
								"insert into addsepvis(visitorname,visitorphoneno,studentname,relation,entrytime,leavingtime,visitingpurpose,day,year) values(?,?,?,?,?,?,?,?,?)");
						ps.setString(1, dn);
						ps.setString(2, dn1);
						ps.setString(3, dn2);
						ps.setString(4, dn3);
