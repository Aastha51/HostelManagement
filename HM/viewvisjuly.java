...
public class viewvisjuly {
	...
	public static void mainNewScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					viewvisjuly window = new viewvisjuly();
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBounds(100, 100, 842, 517);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		JButton btnCancel = new JButton("cancel");
		btnCancel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				visjuly m = new visjuly();
				m.mainNewScreen();
				frame.dispose();
			}
		});
		frame.getContentPane().setLayout(null);
		btnCancel.setBounds(449, 207, 89, 23);
		frame.getContentPane().add(btnCancel);

		JLabel lblViewFloor = new JLabel("view visitors");
		lblViewFloor.setFont(new Font("Tahoma", Font.BOLD, 21));
		lblViewFloor.setBounds(271, 29, 187, 23);
		...
		table.setModel(new DefaultTableModel(
				new Object[][] { { null, null, null, null, null, null, null, null, null },
						...
						{ null, null, null, null, null, null, null, null, null },
						{ null, null, null, null, null, null, null, null, null }, },
				new String[] { "visitorname", "visitorphoneno", "studentname", "relation", "entrytime", "leavingtime",
						"visitingpurpose", "day", "year" }));
		table.setBounds(1, 26, 385, 80);
		frame.getContentPane().add(table);

		JScrollPane scrollPane = new JScrollPane(table);
		scrollPane.setBounds(20, 63, 766, 104);
		frame.getContentPane().add(scrollPane);

		JLabel lblClickYesTo = new JLabel("Click yes to view all visitors details :");
		...
		btnYes.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent g) {
				...
				try {
					...
					ResultSet rs = st.executeQuery("select * from addjulyvis");
		...
		JLabel lblSearchWithBlock = new JLabel("Search with student name :");
		lblSearchWithBlock.setFont(new Font("Tahoma", Font.BOLD, 15));
		lblSearchWithBlock.setBounds(20, 267, 224, 14);
		frame.getContentPane().add(lblSearchWithBlock);

		JLabel label = new JLabel("Search with entry time :");
		label.setFont(new Font("Tahoma", Font.BOLD, 15));
		label.setBounds(30, 297, 217, 27);
		frame.getContentPane().add(label);

		JLabel label_1 = new JLabel("Search with leaving time:");
		label_1.setFont(new Font("Tahoma", Font.BOLD, 15));
		label_1.setBounds(30, 335, 201, 26);
		frame.getContentPane().add(label_1);

		JLabel label_2 = new JLabel("Search with day :");
		label_2.setFont(new Font("Tahoma", Font.BOLD, 15));
		label_2.setBounds(43, 384, 201, 21);
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					java.sql.Connection con = MydbConnectionClass.getConnection();
					String sql = "select * from addjulyvis where studentname=?";
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					java.sql.Connection con = MydbConnectionClass.getConnection();
					String sql = "select * from addjulyvis where entrytime=?";
					java.sql.PreparedStatement pdt = con.prepareStatement(sql);
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					java.sql.Connection con = MydbConnectionClass.getConnection();
					String sql = "select * from addjulyvis where leavingtime=?";
					java.sql.PreparedStatement pdt = con.prepareStatement(sql);
		...
		button_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					java.sql.Connection con = MydbConnectionClass.getConnection();
					String sql = "select * from addjulyvis where day=?";
					java.sql.PreparedStatement pdt = con.prepareStatement(sql);
		...
		button_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					java.sql.Connection con = MydbConnectionClass.getConnection();
					String sql = "select * from addjulyvis where year=?";
					java.sql.PreparedStatement pdt = con.prepareStatement(sql);
		...
		lblSearchWithYear.setBounds(43, 416, 202, 33);
