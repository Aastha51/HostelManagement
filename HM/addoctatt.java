...
public class addoctatt {
	...
	public static void mainNewScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					addoctatt window = new addoctatt();
	...
	private void initialize() {
		...
		JLabel lblAddJanuaryAttendence = new JLabel("add October attendence");
				...
				if (e.getSource() == btnOk) {
					...
					try {
						...
						java.sql.PreparedStatement ps = con.prepareStatement(
								"insert into addoctomatt(studentid,studentname,day,year,status) values(?,?,?,?,?)");
						ps.setString(1, dn);
						ps.setString(2, dn1);
						ps.setString(3, dn2);
						ps.setString(4, dn3);
		...
		btnCancel.setBounds(314, 387, 95, 22);
