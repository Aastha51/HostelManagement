...
public class viewblock {
	...
	public static void mainNewScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					viewblock window = new viewblock();
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBounds(100, 100, 523, 508);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		JButton btnCancel = new JButton("exit");
		btnCancel.setBounds(265, 435, 89, 23);
		...
		JLabel lblViewBlockDetails = new JLabel("view block details");
		lblViewBlockDetails.setBounds(113, 14, 227, 38);
		lblViewBlockDetails.setFont(new Font("Tahoma", Font.BOLD, 22));
		frame.getContentPane().add(lblViewBlockDetails);
		...
		table.setModel(new DefaultTableModel(
				...
				new String[] { "blockid", "blockno", "blockname", "blockstatus" }));
		table.setBounds(20, 31, 404, 216);
		frame.getContentPane().add(table);

		JScrollPane scrollPane = new JScrollPane(table);
		scrollPane.setBounds(20, 63, 404, 104);
		frame.getContentPane().add(scrollPane);

		JLabel lblClickYesTo = new JLabel("Click yes to view all block details :");
		...
		btnYes.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent g) {
				...
				try {
					...
					ResultSet rs = st.executeQuery("select * from addblock");
		...
		JLabel lblSearchWithBlock = new JLabel("Search with block id :");
		...
		JLabel label = new JLabel("Search with block no.:");
		label.setFont(new Font("Tahoma", Font.BOLD, 15));
		label.setBounds(43, 309, 201, 14);
		frame.getContentPane().add(label);

		JLabel label_1 = new JLabel("Search with block name :");
		label_1.setFont(new Font("Tahoma", Font.BOLD, 15));
		label_1.setBounds(43, 347, 201, 14);
		frame.getContentPane().add(label_1);

		JLabel label_2 = new JLabel("Search with block status :");
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					java.sql.Connection con = MydbConnectionClass.getConnection();
					String sql = "select * from addblock where bid=?";
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					java.sql.Connection con = MydbConnectionClass.getConnection();
					String sql = "select * from addblock where blockno=?";
					java.sql.PreparedStatement pdt = con.prepareStatement(sql);
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					java.sql.Connection con = MydbConnectionClass.getConnection();
					String sql = "select * from addblock where blockname=?";
					java.sql.PreparedStatement pdt = con.prepareStatement(sql);
		...
		button_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					java.sql.Connection con = MydbConnectionClass.getConnection();
					String sql = "select * from addblock where blockstatus=?";
					java.sql.PreparedStatement pdt = con.prepareStatement(sql);
