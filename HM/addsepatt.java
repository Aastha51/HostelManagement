...
public class addsepatt {
	...
	public static void mainNewScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					addsepatt window = new addsepatt();
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBounds(100, 100, 801, 460);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.getContentPane().setLayout(null);
		JLabel lblAddJanuaryAttendence = new JLabel("add September attendence");
		lblAddJanuaryAttendence.setFont(new Font("Tahoma", Font.BOLD, 25));
		lblAddJanuaryAttendence.setBounds(80, 95, 370, 37);
				...
				if (e.getSource() == btnOk) {
					...
					try {
						...
						java.sql.PreparedStatement ps = con.prepareStatement(
								"insert into addseptatt(studentid,studentname,day,year,status) values(?,?,?,?,?)");
						ps.setString(1, dn);
						ps.setString(2, dn1);
						ps.setString(3, dn2);
						ps.setString(4, dn3);
		...
		btnCancel.setBounds(303, 387, 89, 23);
