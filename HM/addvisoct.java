...
public class addvisoct {
	...
	public static void mainNewScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					addvisoct window = new addvisoct();
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBounds(100, 100, 806, 476);
				...
				if (e.getSource() == btnOk) {
					...
					try {
						...
						java.sql.PreparedStatement ps = con.prepareStatement(
								"insert into addoctvis(visitorname,visitorphoneno,studentname,relation,entrytime,leavingtime,visitingpurpose,day,year) values(?,?,?,?,?,?,?,?,?)");
						ps.setString(1, dn);
						ps.setString(2, dn1);
						ps.setString(3, dn2);
						ps.setString(4, dn3);
