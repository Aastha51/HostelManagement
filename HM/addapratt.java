...
public class addapratt {
	...
	public static void mainNewScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					addapratt window = new addapratt();
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBounds(100, 100, 744, 458);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.getContentPane().setLayout(null);
		JLabel lblAddJanuaryAttendence = new JLabel("add april attendence");
		lblAddJanuaryAttendence.setFont(new Font("Tahoma", Font.BOLD, 25));
		lblAddJanuaryAttendence.setBounds(133, 88, 323, 37);
		...
		textField_4.setBounds(248, 311, 125, 22);
				...
				if (e.getSource() == btnOk) {
					...
					try {
						...
						java.sql.PreparedStatement ps = con.prepareStatement(
								"insert into addaprilatt(studentid,studentname,day,year,status) values(?,?,?,?,?)");
						ps.setString(1, dn);
						ps.setString(2, dn1);
						ps.setString(3, dn2);
						ps.setString(4, dn3);
		...
		btnCancel.setBounds(301, 353, 89, 23);
