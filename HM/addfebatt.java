...
public class addfebatt {
	...
	public static void mainNewScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					addfebatt window = new addfebatt();
	...
	private void initialize() {
		...
		JLabel lblAddJanuaryAttendence = new JLabel("add february attendence");
				...
				if (e.getSource() == btnOk) {
					...
					try {
						...
						java.sql.PreparedStatement ps = con.prepareStatement(
								"insert into addfebatt(studentid,studentname,day,year,status) values(?,?,?,?,?)");
						ps.setString(1, dn);
						ps.setString(2, dn1);
						ps.setString(3, dn2);
						ps.setString(4, dn3);
